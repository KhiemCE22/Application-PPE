...
void yuv420p_to_bgr_neon(
    const uint8_t* y_src, const uint8_t* u_src, const uint8_t* v_src,
    uint8_t* bgr_dst, int width, int height,
    int y_stride, int uv_stride, int dst_stride) 
{
    // CÁC H?NG S? CHUY?N Ð?I MÀU (Ðã du?c scale lên 128 l?n d? dùng toán s? nguyên)
    // Công th?c g?c: 
    // R = Y + 1.402 * (V - 128)
    // G = Y - 0.344 * (U - 128) - 0.714 * (V - 128)
    // B = Y + 1.772 * (U - 128)
    // Công th?c t?i uu 16-bit NEON (D?ch ph?i 7 bit ~ chia 128):
    const int16x8_t c128 = vdupq_n_s16(128);
    const int16x8_t c_r  = vdupq_n_s16(179); // 1.402 * 128 ˜ 179
    const int16x8_t c_gu = vdupq_n_s16(44);  // 0.344 * 128 ˜ 44
    const int16x8_t c_gv = vdupq_n_s16(91);  // 0.714 * 128 ˜ 91
    const int16x8_t c_b  = vdupq_n_s16(227); // 1.772 * 128 ˜ 227

    // X? lý 2 hàng (row) cùng lúc vì U và V b? thu nh? 1/2 theo c? chi?u d?c và ngang
    for (int y = 0; y < height; y += 2) {
        const uint8_t* y0_ptr = y_src + y * y_stride;
        const uint8_t* y1_ptr = y_src + (y + 1) * y_stride;
        const uint8_t* u_ptr  = u_src + (y / 2) * uv_stride;
        const uint8_t* v_ptr  = v_src + (y / 2) * uv_stride;

        uint8_t* dst0 = bgr_dst + y * dst_stride;
        uint8_t* dst1 = bgr_dst + (y + 1) * dst_stride;

        // X? lý 16 pixel m?i vòng l?p (v?a khít thanh ghi 128-bit)
        for (int x = 0; x < width; x += 16) {
            
            // --- BU?C 1: PREFETCH (N?p tru?c d? li?u vào L1 Cache) ---
            __builtin_prefetch(y0_ptr + x + 64);
            __builtin_prefetch(y1_ptr + x + 64);
            __builtin_prefetch(u_ptr + x/2 + 32);
            __builtin_prefetch(v_ptr + x/2 + 32);

            // --- BU?C 2: X? LÝ KÊNH MÀU U & V ---
            // Ð?c 8 byte U và 8 byte V
            uint8x8_t u_8 = vld1_u8(u_ptr + x / 2);
            uint8x8_t v_8 = vld1_u8(v_ptr + x / 2);

            // Nhân dôi m?i byte d? áp d?ng cho 2 pixel li?n k? (Ví d?: U0 U0 U1 U1...)
            uint8x8x2_t u_zip = vzip_u8(u_8, u_8);
            uint8x8x2_t v_zip = vzip_u8(v_8, v_8);
            uint8x16_t u_16 = vcombine_u8(u_zip.val[0], u_zip.val[1]);
            uint8x16_t v_16 = vcombine_u8(v_zip.val[0], v_zip.val[1]);

            // Chuy?n sang 16-bit và tr? di 128 (Ðua v? kho?ng -128 d?n 127)
            int16x8_t u_lo = vsubq_s16(vreinterpretq_s16_u16(vmovl_u8(vget_low_u8(u_16))), c128);
            int16x8_t u_hi = vsubq_s16(vreinterpretq_s16_u16(vmovl_u8(vget_high_u8(u_16))), c128);
            int16x8_t v_lo = vsubq_s16(vreinterpretq_s16_u16(vmovl_u8(vget_low_u8(v_16))), c128);
            int16x8_t v_hi = vsubq_s16(vreinterpretq_s16_u16(vmovl_u8(vget_high_u8(v_16))), c128);

            // Tính toán chung các sai s? màu (Dùng chung cho c? Hàng 0 và Hàng 1)
            // vshrq_n_s16(..., 7) tuong duong phép chia cho 128
            int16x8_t r_term_lo = vshrq_n_s16(vmulq_s16(v_lo, c_r), 7);
            int16x8_t g_term_lo = vshrq_n_s16(vaddq_s16(vmulq_s16(u_lo, c_gu), vmulq_s16(v_lo, c_gv)), 7);
            int16x8_t b_term_lo = vshrq_n_s16(vmulq_s16(u_lo, c_b), 7);

            int16x8_t r_term_hi = vshrq_n_s16(vmulq_s16(v_hi, c_r), 7);
            int16x8_t g_term_hi = vshrq_n_s16(vaddq_s16(vmulq_s16(u_hi, c_gu), vmulq_s16(v_hi, c_gv)), 7);
            int16x8_t b_term_hi = vshrq_n_s16(vmulq_s16(u_hi, c_b), 7);

            // --- BU?C 3: X? LÝ HÀNG TRÊN (ROW 0) ---
            uint8x16_t y0 = vld1q_u8(y0_ptr + x);
            int16x8_t y0_lo = vreinterpretq_s16_u16(vmovl_u8(vget_low_u8(y0)));
            int16x8_t y0_hi = vreinterpretq_s16_u16(vmovl_u8(vget_high_u8(y0)));

            // Tính B, G, R (G b? tr? di, R và B du?c c?ng vào)
            int16x8_t b0_lo = vaddq_s16(y0_lo, b_term_lo);
            int16x8_t g0_lo = vsubq_s16(y0_lo, g_term_lo);
            int16x8_t r0_lo = vaddq_s16(y0_lo, r_term_lo);

            int16x8_t b0_hi = vaddq_s16(y0_hi, b_term_hi);
            int16x8_t g0_hi = vsubq_s16(y0_hi, g_term_hi);
            int16x8_t r0_hi = vaddq_s16(y0_hi, r_term_hi);

            // vqmovun_s16: T? d?ng k?p giá tr? (clamp) vào kho?ng 0-255 và chuy?n v? 8-bit
            uint8x16x3_t bgr0;
            bgr0.val[0] = vcombine_u8(vqmovun_s16(b0_lo), vqmovun_s16(b0_hi)); // Kênh B
            bgr0.val[1] = vcombine_u8(vqmovun_s16(g0_lo), vqmovun_s16(g0_hi)); // Kênh G
            bgr0.val[2] = vcombine_u8(vqmovun_s16(r0_lo), vqmovun_s16(r0_hi)); // Kênh R
            
            // Luu xen k? B-G-R vào RAM
            vst3q_u8(dst0 + x * 3, bgr0);

            // --- BU?C 4: X? LÝ HÀNG DU?I (ROW 1) ---
            // Tuong t? Row 0, nhung dùng Y c?a Row 1, TÁI S? D?NG term màu c?a Row 0
            uint8x16_t y1 = vld1q_u8(y1_ptr + x);
            int16x8_t y1_lo = vreinterpretq_s16_u16(vmovl_u8(vget_low_u8(y1)));
            int16x8_t y1_hi = vreinterpretq_s16_u16(vmovl_u8(vget_high_u8(y1)));

            int16x8_t b1_lo = vaddq_s16(y1_lo, b_term_lo);
            int16x8_t g1_lo = vsubq_s16(y1_lo, g_term_lo);
            int16x8_t r1_lo = vaddq_s16(y1_lo, r_term_lo);

            int16x8_t b1_hi = vaddq_s16(y1_hi, b_term_hi);
            int16x8_t g1_hi = vsubq_s16(y1_hi, g_term_hi);
            int16x8_t r1_hi = vaddq_s16(y1_hi, r_term_hi);

            uint8x16x3_t bgr1;
            bgr1.val[0] = vcombine_u8(vqmovun_s16(b1_lo), vqmovun_s16(b1_hi));
            bgr1.val[1] = vcombine_u8(vqmovun_s16(g1_lo), vqmovun_s16(g1_hi));
            bgr1.val[2] = vcombine_u8(vqmovun_s16(r1_lo), vqmovun_s16(r1_hi));
            
            vst3q_u8(dst1 + x * 3, bgr1);
        }
    }
}
