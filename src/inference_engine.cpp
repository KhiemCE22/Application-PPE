...
ErrorCode InferenceEngine::infer_fp32(const float* input_data, DetectionResult& result) {
    ...
    //std::cout << "INFERENCE.CPP: LINE 206 <in infer_fp32, >" << "num_proposals = " << num_proposals << "; num_channels = " << num_channels << "\n";
    ...
    for (int i = 0; i < num_channels; i++) {
        for (int j = 0; j < num_proposals; j++) {
            output_buffer_.get()[j * num_channels + i] = ((float*)output.data)[i * num_proposals + j];
        }
    }
    */
    
    transpose_general_asm(
        (const float*)output.data,
        output_buffer_.get(),
        num_proposals,
        num_channels
