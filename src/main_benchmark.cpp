...
int run_inference_pipeline(const Options& opts) {
    ...
    auto process_frame = [&](const FrameBuffer& frame) -> bool {
       ...
       //  AlignedPtr<__fp16> model_input_fp16 = make_aligned_buffer<__fp16>(MODEL_INPUT_SIZE);

	// nếu đang có float* model_input, convert:
	//for (size_t i = 0; i < MODEL_INPUT_SIZE; ++i) {
    	   // model_input_fp16.get()[i] = (__fp16)model_input.get()[i];
	//}
