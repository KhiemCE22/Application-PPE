...
// static const char** COCO_NAMES = CLASS_NAMES;
...
public:
    ...
    static void draw_roi(cv::Mat& frame, const GateROI& roi) {
        if (frame.empty()) return;

        // Chuyển đổi tọa độ chuẩn hóa (0-1) sang pixel
        int rx1 = static_cast<int>(roi.x1 * frame.cols);
        int ry1 = static_cast<int>(roi.y1 * frame.rows);
        int rx2 = static_cast<int>(roi.x2 * frame.cols);
        int ry2 = static_cast<int>(roi.y2 * frame.rows);
...
// ============================================================================
// BBox Renderer - Optimized with Track_ID & Violation highlighting
// ============================================================================
/*
class BBoxRenderer {
public:
    ...
    static void draw(cv::Mat& frame, const DetectionResult& result, 
        ...
        for (int i = 0; i < result.count; i++) {
            ...
            // ----------------------------------------------------------------
            // 1. EXTRACT TRACKING & VIOLATION STATE
            // ----------------------------------------------------------------
            int raw_class_id = det.class_id;
            bool is_tracked_worker = false;
            bool is_violating = false;
            int track_id = -1;
            int display_class_id = raw_class_id;

            // Check if this is a tracked person (ID offset by 1000 or negative)
            if (raw_class_id <= -1000 || raw_class_id >= 1000) {
                is_tracked_worker = true;
                
                if (raw_class_id < 0) {
                    is_violating = true;
                    track_id = std::abs(raw_class_id) - 1000; // Remove negative and offset
                } else {
                    track_id = raw_class_id - 1000;           // Remove offset
                }
                
                display_class_id = 5; // Fallback to "Person" class index for safety
            }

            // ----------------------------------------------------------------
            // 2. CALCULATE BOX COORDINATES
            // ----------------------------------------------------------------
            int x1 = static_cast<int>(det.x1 * scale_x);
            int y1 = static_cast<int>(det.y1 * scale_y);
            int x2 = static_cast<int>(det.x2 * scale_x);
            ...
            // ----------------------------------------------------------------
            // 3. DETERMINE COLORS AND DRAW BOX
            // ----------------------------------------------------------------
            cv::Scalar box_color;
            int box_thickness = 2;

            if (is_tracked_worker) {
                if (is_violating) {
                    box_color = cv::Scalar(0, 0, 255);   // Pure RED for Violations
                    box_thickness = 3;                   // Thicker box for emphasis
                } else {
                    box_color = cv::Scalar(0, 255, 0);   // Pure GREEN for Safe workers
                }
            } else {
                // Standard PPE classes colors
                box_color = CLASS_COLORS[display_class_id % NUM_COLORS];
            }

            cv::rectangle(frame, cv::Point(x1, y1), cv::Point(x2, y2), box_color, box_thickness);
            
            // ----------------------------------------------------------------
            // 4. FORMAT LABEL STRING
            // ----------------------------------------------------------------
            char label[64];
            if (is_tracked_worker) {
                if (is_violating) {
                    snprintf(label, sizeof(label), "Worker %d: VIOLATION", track_id);
                } else {
                    // snprintf(label, sizeof(label), "Worker %d: SAFE", track_id);
                    snprintf(label, sizeof(label), "%d", track_id); // short lable for tracking
                }
            } else {
                // Ensure array bounds safety for PPE labels
                int safe_class_idx = display_class_id % (sizeof(CLASS_NAMES) / sizeof(CLASS_NAMES[0]));
                snprintf(label, sizeof(label), "%s %.0f%%", 
                         CLASS_NAMES[safe_class_idx], 
                         det.confidence * 100);
            }
            ...
            // Text color logic: White text by default, Black text on bright backgrounds (like Green)
            cv::Scalar text_color = (is_tracked_worker && !is_violating) ? 
